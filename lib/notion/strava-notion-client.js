...
class NotionClient {
  constructor() {
    this.notion = new Client({ auth: process.env.STRAVA_NOTION_TOKEN });
    this.databaseId = process.env.STRAVA_NOTION_DATABASE_ID;
  }
  ...
  async checkExistingActivityRecord(activityId) {
    try {
      const response = await this.notion.databases.query({
        database_id: this.databaseId,
        filter: {
          property: "Activity ID",
          number: { equals: activityId },
        },
      });
      ...
      console.error(
        "❌ Error checking existing activity record:",
        error.message
      );
      return null;
    }
  ...
  async createWorkoutRecord(workoutData) {
    try {
      // Check if record already exists via Activity ID
      if (workoutData && typeof workoutData.id !== "undefined") {
        const existing = await this.checkExistingActivityRecord(workoutData.id);
        if (existing) {
          console.log(
            `⏭️  Skipping duplicate Strava record: ${workoutData.name || "Unnamed Workout"} (ID: ${workoutData.id})`
          );
          return null;
        }
      }
